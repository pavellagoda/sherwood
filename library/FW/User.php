	...
	public static function getLoggedUser ()
	{
		...
		else
		{
			if ( self::isLogged() )
			{
				...
				self::$loggedUser = models_UsersMapper::findById( $aIdentity['User_id'] ); 
...
//				$oTranslate = Zend_Registry::get('Zend_Translate');
			...
			else
			{
				...
				FW_User::setLogged($stdClassUser->id);
	...
	public static function activate($iUserId)
    {
        ...
        if ( models_UsersMapper::edit($iUserId, 
        	array('status' => models_Users::STATUS_ACTIVE)) )
        {
			return true;        		
        }
...
//------------------------------------------------------------------------------
	...
	{
		return Lemar_File::getOrMakeDirPathForUploadedFile($teacher->getIdUser(), 
			Zend_Registry::get('config')->photo->directory, false) . 
	...
	public static function makePhotoPreview( models_Teachers $teacher )
	{
		...
		$imageName = FW_User::getTeacherPhotoFilePath($teacher);
